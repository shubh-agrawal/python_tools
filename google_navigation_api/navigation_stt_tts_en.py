...
def ask_for_user_data():
	...
	while 1:	
		espeak.synth("Enter the mode of transport, Empty input defaults to driving, Available options driving, bicycling, walking. ")
		...
		if len(mode)<=1:
			...
			espeak.synth("Something went wrong. Its a typo mistake. Enter data once again")
			continue
...
def get_primary_data(raw_data):
	...
	espeak.synth("Start address is, "+primary_data_dict['start_address_api_set'])
	
	print "End address :",primary_data_dict['end_address_api_set']
	espeak.synth("End address is, "+primary_data_dict['end_address_api_set'])
